...
public KinectHelper helper = new KinectHelper();
...
public void setup() {
  ...
  backgroundImg = helper.createGradientImg(this, color(240), color(150));
...
public void draw() {
  ...
  if (takeScan) {
    if (framesPerScan == 1) {
      ...
      if (frameIterator >= framesPerScan) {
        scan = helper.averageScans(scansToAverage);
      }
    }
  ...
  if (takeSlitScan) {
    ...
    slitScan = helper.combineSlits(slits, rotateSlitScan, centerSlitScan);
